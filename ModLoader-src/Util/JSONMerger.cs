...
namespace BattleTech.ModSupport.Utils
{
    ...
    {
        public static readonly HBS.Logging.ILog jsonMergeLogger =
            HBS.Logging.Logger.GetLogger(HBS.Logging.LoggerNames.JSONMERGER, HBS.Logging.LogLevel.Log);
        ...
        private static void DoAdvancedMerge(JObject target, JObject merge)
        {
            ...
            foreach (var instruction in instructions)
            {
                if (!instruction.Process(target))
                    jsonMergeLogger.Log($"Warning: An instruction (Action: '{instruction.Action}' JSONPath: '{instruction.JSONPath}') did not perform anything.");
            }
        }
    ...
    public class AdvancedJSONMerge
    {
        ...
        public static AdvancedJSONMerge FromFile(string path)
        {
            ...
            {
                jsonMergeLogger.LogException($"\tCould not read AdvancedJSONMerge in path: {path}", e);
                return null;
            }
        }
    }
