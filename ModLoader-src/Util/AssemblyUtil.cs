...
namespace BattleTech.ModSupport.Utils
{
    internal static class AssemblyUtil
    {
        ...
        public static Assembly LoadDLL(string path)
        {
            ...
            if (!File.Exists(path))
            {
                modLogger.Log($"\tFailed to load {fileName} at path {path}, because it doesn't exist at that path.");
                return null;
            }
            ...
            try
            {
                ...
                modLogger.Log($"\tLoaded assembly {assembly.GetName().Name} (v{assembly.GetName().Version})");
                return assembly;
            }
            catch (Exception e)
            {
                modLogger.LogException($"\t{fileName}: While loading a .dll, an exception occured", e);
                return null;
            }
        }
        ...
        public static bool InvokeMethodByParameterNames(MethodInfo method, Dictionary<string, object> paramsDictionary)
        {
            ...
            {
                modLogger.Log($"\tInvoking '{method.DeclaringType?.Name}.{method.Name}()' using parameter dictionary");
            ...
            modLogger.Log($"\tInvoking '{method.DeclaringType?.Name}.{method.Name}({parametersString})' using parameter dictionary");
        ...
        public static bool InvokeMethodByParameterTypes(MethodInfo method, object[] parameters)
        {
            ...
            if (parameters == null)
            {
                ...
                modLogger.Log($"\tInvoking '{method.DeclaringType?.Name}.{method.Name}()' using parameter type");
                method.Invoke(null, null);
                return true;
            }
            ...
            var parametersString = string.Join(", ", parametersStrings.ToArray());
            modLogger.Log($"\tInvoking '{method.DeclaringType?.Name}.{method.Name}({parametersString})' using parameter type");
